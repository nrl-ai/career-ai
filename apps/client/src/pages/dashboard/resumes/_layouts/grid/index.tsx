      ...
      {maxSize && resumes && resumes.length > maxSize && (
        <Link to="/dashboard/resumes">
          <motion.div
            ...
            animate={{ opacity: 1, x: 0, transition: { delay: 0.1 } }}
          >
            <BaseCard onClick={() => { }} className="bg-gray-100 text-gray-500 border-2 border-dashed border-gray-500 flex items-center justify-center">
              <div className="flex flex-col items-center justify-center text-center p-2">
                <MdKeyboardArrowRight className="w-12 h-12 mx-auto" />
                <h4 className="text-gray-500 font-bold mt-8">
                  {`View ${resumes.length - maxSize} more`}
                </h4>
            </div>
            </BaseCard>
          </motion.div>
