...
class TalkingHead {
  ...
  constructor(node, opt = null) {
    ...
    this.animMoods = {
      ...
      disgust: {
        ...
        anims: [
          ...
          { name: "pose", alt: [{ delay: [5000, 10000], vs: { pose: ["side"] } }] },
          {
            name: "head",
            idle: {
              delay: [100, 500],
    ...
    this.animEmojis = {
      ...
      "😴": {
        dt: [5000, 5000],
        vs: { eyeBlinkLeft: [1], eyeBlinkRight: [1], headRotateX: [0.2], headRotateZ: [0.1] },
      },
      ...
      yes: {
        ...
        vs: { headRotateX: [[0.1, 0.2], 0.1, [0.1, 0.2], 0], headRotateZ: [[-0.2, 0.2]] },
      },
      no: {
        dt: [
          [200, 500],
    ...
    for (let i = 0; i < this.animQueue.length; i++) {
      ...
      if (this.animClock >= x.ts[0]) {
        ...
        if (this.animClock >= x.ts[x.ts.length - 1]) {
          ...
          if (x.loop) {
            let restrain =
              this.isSpeaking && (x.template.name === "head" || x.template.name === "eyes") ? 4 : 1;
    ...
    if (this.cameraClock !== null && this.cameraClock < 1000) {
      ...
      if (this.controlsStart.x !== this.controlsEnd.x) {
        this.controls.target.copy(
          this.controlsStart.lerp(this.controlsEnd, this.easing(this.cameraClock / 1000)),
        );
      } else {
                ...
                for (let j = 0; j < v.visemes.length; j++) {
                  ...
                  lipsyncAnim.push({
                    ...
                    vs: {
                      ["viseme_" + v.visemes[j]]: [
                        null,
                        v.visemes[j] === "PP" || v.visemes[j] === "FF" ? 0.9 : level,
                        0,
                      ],
                    },
                  });
      ...
      if (line.emoji) {
        ...
        try {
          ...
          const res = await fetch(
            this.opt.ttsEndpoint + (this.opt.ttsApikey ? "?key=" + this.opt.ttsApikey : ""),
            o,
          );
