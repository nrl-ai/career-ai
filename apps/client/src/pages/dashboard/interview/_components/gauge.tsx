    ...
    <div className={cn("relative size-70", className)}>
      <svg className="size-full rotate-180" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg">
        ...
        <circle
          ...
          stroke-width="3"
          stroke-dasharray={`${(((value * 1) / 10) * 1) / 2} 100`}
          stroke-linecap="round"
        ></circle>
      </svg>

      <div className="absolute top-16 start-1/2 transform -translate-x-1/2 text-center">
        <span className={cn("text-[60px] text-[#34c759] font-semibold", valueClassName)}>
          {value}
        </span>
        <span className={cn("text-xl font-semibold text-[#191919] block mt-3", labelClassName)}>
          {label}
        </span>
      </div>
    </div>
